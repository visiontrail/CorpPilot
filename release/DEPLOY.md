# CostMatrix Docker 离线发布包使用指南

本指南面向公司内部 IT 工程师，说明如何在一台 Linux 服务器上部署 / 升级 / 回滚 CostMatrix。部署无需访问互联网（镜像已在发布包内）。

## 1. 前置条件
- 已安装 Docker 20+；已安装 Docker Compose v2（推荐）或 v1。
- 服务器具备基础权限可运行 Docker，磁盘预留 ≥ 5GB。
- 端口：`8180`（前端）与 `8000`（后端 API）可用；如需修改请在 `.env` 中调整。
